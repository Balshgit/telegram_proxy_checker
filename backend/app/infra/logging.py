...
from settings.infra import LogLevelEnum
...
def configure_logging(*, level: LogLevelEnum, enable_json_logs: bool) -> None:
    ...
    pool_logger = logging.getLogger("sqlalchemy.pool.impl.AsyncAdaptedQueuePool.postgres")
    engine_logger = logging.getLogger("sqlalchemy.engine.Engine.postgres")

    # не прокидываем записи в родительский логер, чтобы не дублировать логи
    pool_logger.propagate = False
    engine_logger.propagate = False

    pool_logger.handlers = [intercept_handler]
    engine_logger.handlers = [intercept_handler]
